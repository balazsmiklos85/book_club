...
public class ActiveUserService extends AuthenticatedService {
  ...
  public User getUser() {
    return userRepository.findById(getUserId()).orElseThrow(NotAuthenticatedException::new);
  }
   ...
   */
  public boolean isCurrentUser(@Nullable Member member) {
    return nonNull(member) && isCurrentUser(member.externalId());
  }
}
