...
public class DeprecatedSuggestionController {
  ...
  public DeprecatedSuggestionController(
      ActiveUserService activeUserService, SuggestionService suggestionService) {
  ...
  public ModelAndView getSuggestion(@PathVariable UUID id) {
    ...
    SuggestionFormData suggestionData =
        new SuggestionDtoToSuggestionFormDataConverter(id).convert(bookAndSuggestion);
    UUID suggesterId = suggestion.map(SuggestionDto::getSuggester).map(UserDto::getId).orElse(null);
    String viewName =
        "suggestion/" + (activeUserService.isCurrentUser(suggesterId) ? "edit" : "show");
    return new ModelAndView(viewName, "suggestion", suggestionData);
  }

  @PostMapping("/{id}")
  public ModelAndView updateSuggestion(
      @PathVariable UUID id, @ModelAttribute SuggestionFormData suggestionData) {
  ...
  private Optional<SuggestionDto> getSuggestion(BookAndSuggesterDto bookAndSuggestion, UUID id) {
    return bookAndSuggestion.suggestions().stream()
        .filter(
            suggestion ->
                id.equals(suggestion.getId())
                    || suggestion.getId() == null && id.equals(bookAndSuggestion.book().getId()))
        .findFirst();
  }
}
