...
public class SuggestionServiceImpl extends AuthenticatedService implements SuggestionService {
  ...
  public void createSuggestion(CreateBookRequest book) {
    ...
    var suggestion =
        new Suggestion(createdBook.getId(), getUserId(), Instant.now(), book.getDescription());
    ...
    if (storedSuggestion.isPresent()) {
      ...
      if (storedBook.isPresent()) {
        ...
        if (activeUserService.isCurrentUser(bookRecommender.getId())
            || activeUserService.isCurrentUser(book.getRecommenderExternalId())) {
          var suggestion =
              new Suggestion(
                  bookId, getUserId(), book.getRecommendedAt(), suggestionData.getDescription());
          suggestionRepository.saveAndFlush(suggestion);
          ...
          throw new IllegalArgumentException(
              "You can only edit your own suggestions."); // TODO this should be controller
          // responsibility!
        }
      } else {
