...
public class VoteByBookCollector
    implements Collector<Vote, Map<UUID, Collection<UserDto>>, Map<UUID, Collection<UserDto>>> {
  ...
  public BiConsumer<Map<UUID, Collection<UserDto>>, Vote> accumulator() {
    return (map, vote) -> {
      final User voter = Optional.ofNullable(vote.getUserById()).orElse(vote.getUserByExternalId());
      if (voter != null) {
          ...
          .forEach(
              map ->
                  map.entrySet().stream()
                      .forEach(
                          entry -> {
                            Collection<UserDto> voters =
                                result.getOrDefault(entry.getKey(), new ArrayList<>());
