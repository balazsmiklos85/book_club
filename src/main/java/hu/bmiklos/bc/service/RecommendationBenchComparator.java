...
/**
 * A comparator for sorting books based on whether they are in the top 5 recommended books for any
 * user. The books are sorted in descending order, so the top 5 books are first.
 *
 * <p>This is done to implement what was the "recommendation bench" in the old Excel sheet: each
 * user could only have 5 books for voting, the rest of them went to the recommendation bench.
 */
public class RecommendationBenchComparator implements Comparator<Book> {
  ...
  /**
   * This class represents a comparator for book recommendations. It compares books based on whether
   * they are in the top 5 recommended books for any user.
   *
   * @param books A list of books to be used for comparison. The sorting order of the list is used
   *     to determine the top 5 status.
   */
  public RecommendationBenchComparator(List<Book> books) {
    this.userTop5 = new HashMap<>();
    this.historicUserTop5 = new HashMap<>();
    for (Book book : books) {
      Collection<User> suggesters =
          book.getSuggestions().stream().map(Suggestion::getSuggester).toList();
      if (suggesters == null || suggesters.isEmpty()) {
        suggesters = Optional.ofNullable(book.getRecommender()).map(List::of).orElse(List.of());
      }
      Integer historicRecommender = book.getRecommenderExternalId();
      if (suggesters.isEmpty()) {
        List<UUID> result;
        Optional<User> userByHistory =
            userTop5.keySet().stream()
        ...
        for (User recommender : suggesters) {
          ...
          if (recommender != null && recommender.getId() != null) {
            ...
            if (userByHistory.isPresent()) {
              ...
              result =
                  historicUserTop5.computeIfAbsent(historicRecommender, k -> new ArrayList<>());
            }
          }
          List<UUID> top5 = result;
          if (top5.size() < 5) {
