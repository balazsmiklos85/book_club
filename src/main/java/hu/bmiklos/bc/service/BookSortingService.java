...
public class BookSortingService {
   ...
   *
   * <p>The recommendation score is calculated based on the votes, and the previous events which the
   * voters attended. Books that already have a previous event declared are excluded from the list.
   * Books that are not in the top 5 recommended books for their recommender, are punished, and
  ...
  public List<BookAndSuggesterDto> getAll() {
    ...
    Stream<Book> booksWithoutEvents =
        allBooks.stream().filter(new ShownOnLeaderboardCondition()::test);
    List<Book> preSortedBooks = booksWithoutEvents.sorted(new BookComparator(bookWeights)).toList();
    Stream<Book> sortedBooks =
        preSortedBooks.stream().sorted(new RecommendationBenchComparator(preSortedBooks));
    return sortedBooks.map(new BookToBookAndSuggesterDtoConverter()::convert).toList();
  }
}
