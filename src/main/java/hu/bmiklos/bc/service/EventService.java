  ...
  public UUID createEvent(CreateEventDto event) {
    UUID hostId =
        event
            .getHostId()
            .orElseThrow(
                () -> new IllegalArgumentException("The host ID is required for new events."));
  ...
  public GetEventDto editEvent(EditEventDto event) {
    Event storedEvent =
        eventRepository
            ...
            .orElseThrow(
                () -> new IllegalArgumentException("No event found with ID " + event.getId()));
        ...
        .map(
            lastEventTime ->
                lastEventTime.plus(
                    4 * 7l,
                    ChronoUnit
                        .DAYS)); // java.time.temporal.UnsupportedTemporalTypeException: Unsupported
    // unit: Weeks
  }
