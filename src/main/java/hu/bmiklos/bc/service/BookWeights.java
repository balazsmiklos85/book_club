...
public class BookWeights {
  ...
  public BookWeights(Collection<Vote> votes, UserWeights userWeights) {
    ...
    weights =
        votes.stream()
            .collect(
                Collectors.groupingBy(Vote::getBook, Collectors.summingLong(this::getUserWeight)));
  }
  ...
  private Long getUserWeight(Vote vote) {
    return max(
        userWeights.getWeight(vote.getUserById()), userWeights.getWeight(vote.getUserExternalId()));
  }
}
