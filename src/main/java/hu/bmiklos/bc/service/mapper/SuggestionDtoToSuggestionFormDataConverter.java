...
public class SuggestionDtoToSuggestionFormDataConverter
    implements Converter<BookAndSuggesterDto, SuggestionFormData> {
  ...
  public SuggestionFormData convert(BookAndSuggesterDto source) {
    ...
    String suggestionId =
        getSuggester(source).map(SuggestionDto::getId).map(Object::toString).orElse(bookId);
    String author = source.book().getAuthor();
    String title = source.book().getTitle();
    String url = source.book().getUrl();
    String description = getSuggester(source).map(SuggestionDto::getDescription).orElse("");
  ...
  private Optional<SuggestionDto> getSuggester(BookAndSuggesterDto source) {
    return source.suggestions().stream()
        .filter(
            suggestion -> {
              UUID suggesterUserId =
                  Optional.ofNullable(suggestion).map(SuggestionDto::getId).orElse(null);
