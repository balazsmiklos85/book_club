...
class SuggestionDtoToSuggestionFormDataConverterTest {
  ...
  void doesNotFindSuggestionsFromOthers() {
    ...
    suggestions.add(
        new SuggestionDto(
            targetSuggestionId, Instant.now(), mock(UserDto.class), "Test description"));
    var bookAndSuggestions =
        new BookAndSuggesterDto(
            new BookDto(UUID.randomUUID(), "Test Author", "Test Title", "http://test.url/for/book"),
            suggestions);
    ...
    assertEquals(
        ...
        "The description should be found by the user ID.");
  }
}
